...
/// URLSession 기반 NetworkProvider 구현체 (Moya의 MoyaProvider와 유사!)
 final class NetworkProvider: NetworkProviding {
    ...
    // MARK: - NetworkProviding
     ...
     //원래는 URLSession.shared.dataTask(with: URLRequest()) { data response ... 이런식으로 했을거야
     
    /// Swift Concurrency를 사용한 네트워크 요청
    /// - Parameter target: TargetType 프로토콜을 준수하는 요청 객체
    /// - Returns: Decodable 타입으로 디코딩된 응답 데이터
    public func request<T: Decodable>(_ target: TargetType) async throws -> T { //모든 네트워크 통신은 이걸 통하게 할거야? 패스 넣어주고 url, 바디 헤더 넣어주고를 하나의 추상화.. 그래서 targettype을 받음 // 모든게 다 이 request를 거친다?
        do {
            // 1. URLRequest 생성
            let urlRequest = try target.toURLRequest() //targettype채택받는 애는 toURLRequest가 있으니 URLrequest 객체를 만들어줌
            ...
            let (data, response) = try await session.data(for: urlRequest) //data라는 메소드 <= concurrency
            ...
            do {
                let decodedData = try JSONDecoder().decode(T.self, from: data) //데이터를 받는다 <- 이걸 제네릭으로 해놓은거
